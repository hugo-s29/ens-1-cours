...
\begin{document}
  ...
  \begin{itemize}
    \item \textsc{Dedekind} (1988) et \textsc{Peano} (1889) formalisent l'arithmétique.
      %QV : Est-ce que tu as la date ?
      %HS : Ça se trouve, mais est-ce que c'est utile d'avoir la date exacte ?
    \item En 1900, David \textsc{Hilbert}, lors du 2ème ICM à Paris, donne un programme dont le 2nd problème est la \textit{cohérence de l'arithmétique}.
  ...
  \begin{rmk}[Convention]
    On note $\mathds{N}$ la $\mathcal{L}_0$-structure de domaine $\mathds{N}$ dans laquelle on interprète les symboles de manière habituelle :
    \begin{itemize}
  ...
  L'ensemble $\mathcal{P}$ est composé de $\mathcal{P}_0$ un ensemble fini d'axiomes (\peano{A1}--\peano{A7}) et d'un schéma d'induction (\peano{SI}).
  ...
  \phantomsection
  \begin{description}
    \item[A5.] \label{peano-A5} $\forall x \: \forall y \: \left(x \oplus (\succ y) = \succ(x \oplus y)\right)$
  \end{description}
  ...
  \[
    ...
    \left(
    \begin{array}{l}
    \quad F(\zero, x_1, \ldots, x_n) \to\\
    \quad \forall x \: \left(F(x, x_1, \ldots, x_n) \to F(\succ x, x_1, \ldots, x_n)\right) \to\\
    \quad \forall x \: F(x, x_1, \ldots, x_n).
  \end{array}
    \right)
  \]
  ...
  \begin{rmk}
    \begin{itemize}
      \item Le schéma est le \peano{SI} avec hypothèse faible, qui permet de montrer le \peano{SI} avec hypothèse forte.
        On adopte la notation $\forall y \le x, \: F(y, x_1, \ldots, x_n)$ pour \[
        \forall y \: \big( (\exists z \:  z \oplus y = x) \to  F(y, x_1, \ldots, x_n) \big)
        .\]
        Le \peano{SI} avec hypothèse forte est :
    \end{itemize}
    \fitbox{
      $\forall x_1 \: \cdots \: \forall x_n \:
      \Big(\forall x \: \big((\forall y < x, \: F(y, x_1, \ldots, x_n)) \to F(x, x_1, \ldots, x_n)\big)
    ...
    \begin{itemize}
      ...
      \item On peur remplacer le \peano{SI} par une nouvelle règle de démonstration :
        \[
        \begin{prooftree}
  ...
  \begin{exo}
    Montrer l'équivalence entre \peano{SI} et la nouvelle règle $\mathsf{rec}$, \textit{i.e.} on peut démontrer les mêmes théorèmes.
  \end{exo}
  ...
  \begin{prv}
    \begin{enumerate}
      ...
      \item Autre preuve, on considère un symbole de constante $c$ et on pose $\mathcal{L} := \mathcal{L}_0 \cup \{c\}$.
        ...
        Soit $T' \subseteq_\mathrm{fini} T$. On dispose de $n \in \mathds{N}$ tel que si $\lnot(c = \repr{k}) \in T'$, alors $k \le n$.
        On étend l'interprétation $\mathds{N}$ de $\mathcal{L}_0$ en une interprétation de $\mathcal{L}$ en posant $c_{\mathds{N}} := n + 1$.
        Alors, $\mathds{N}$ est un modèle de $T'$.
        Ainsi, $T'$ est satisfiable et donc $T$ aussi avec un modèle $\mathcal{M}$.

        Montrons que $\mathds{N}$ et $\mathcal{M}$ ne sont pas $\mathcal{L}_0$-isomorphes.
        Par l'absurde, supposons qu'on ait un isomorphisme $\varphi : \mathcal{M} \to \mathds{N}$.
        Alors $n = \varphi(c_{\mathcal{M}})$ satisfait les mêmes formules que $c_{\mathcal{M}}$, par exemple, $\mathcal{M}, (\_ \mapsto c_{\mathcal{M}}) \models \lnot (x = \repr{n})$.
        Or, on ne peut pas avoir $\mathds{N}, (\_ \mapsto n) \models \lnot (x = \repr{n})$.
        \textbf{\textit{Absurde.}}
    \end{enumerate}
  \end{prv}

  On a montré au passage que tous les modèles isomorphes à $\mathds{N}$ n'ont que des éléments standards.
  ...
  \begin{thm}
    ...
    \begin{enumerate}
      ...
      \item tout élément non nul est régulier pour la multiplication : 
        \[
        \mathcal{M} \models \forall x\: \forall y \: \forall z \: (\lnot (x = \zero) \wedge x \otimes y = x \otimes z \to y = z)
        \;;\]
      \item la formule suivante définit un ordre total sur $\mathcal{M}$ compatible avec $+$ et $\times$ :
        \[
  ...
  \begin{prv}
    ...
    \begin{enumerate}
      \item \label{item:plus-comm-1}
        On montre $\mathcal{P} \vdash \forall x \: (\zero \oplus x = x)$.
        On utilise le \peano{SI} avec la formule $F(x) := (\zero \oplus x = x)$.
        \begin{itemize}
          \item On a $\mathcal{P} \vdash \zero \oplus \zero = \zero$ par \peano{A4}.
          \item On montre $\mathcal{P} \vdash \forall x \: (F(x) \to F(\succ x))$, c'est à dire :
            ...
            .\]
            On peut le montrer par \peano{A5}.
        \end{itemize}
        ...
        \end{itemize}
      \item \label{item:plus-comm-2}
        On montre $\mathcal{P} \vdash \forall x \: \forall y \: (\succ(x \oplus y) = (\succ x) \oplus y)$.
        On veut utiliser le \peano{SI} avec $F(x, y) := \succ(x \oplus y) = (\succ x) \oplus y$.
        ...
        \begin{itemize}
          \item On montre $\mathcal{P} \vdash F(\zero, x)$, c'est à dire 
            ...
            Ceci est vrai car 
            \[
              \succ(x \oplus \zero) \underset {\text{\peano{A4}}} = \succ x \underset {\text{\peano{A4}}} = (\succ x) \oplus \zero
            .\]
          \item De plus, on a aussi $\mathcal{P} \vdash F(y, x) \to F(\succ y, x)$ car : si $\succ(x \oplus y) = (\succ x) \oplus y$, alors 
        \end{itemize}
        \[\begin{array}{rcl}
          \succ(x \oplus (\succ y))&\underset {\text{\peano{A5}}} =&\succ \succ (x \oplus y)\\
          &\underset{\text{hyp}} =&\succ((\succ x) \oplus y)\\
          &\underset {\text{\peano{A5}}} =&(\succ x) \oplus (\succ y).
        \end{array}\]
      \item On utilise le \peano{SI} avec $F(x, y) := (x \oplus y = y \oplus x)$.
        D'une part, on a $F(\zero, y) = (\zero \oplus y = y \oplus \zero)$ par \ref{item:plus-comm-1} et \peano{A4}.
        D'autre part, si l'on a $x \oplus y = y \oplus x$ alors  $(\succ x) \oplus y = y \oplus (\succ x)$ par \peano{A5} et~\ref{item:plus-comm-2}.
        Par le \peano{SI}, on conclut.
    \end{enumerate}
  \end{prv}
  ...
  \begin{defn}
     Soient $\mathcal{M} \models \mathcal{P}_0$ et $\mathcal{N} \models \mathcal{P}_0$ avec $\mathcal{N}$ une sous-interprétation de $\mathcal{M}$, on dit que $\mathcal{N}$ est un segment initial de $\mathcal{M}$, ou que $\mathcal{M}$ est une extension finale de $\mathcal{N}$, si pour tous $a,b,c \in |\mathcal{M}|$ avec $a \in |\mathcal{N}|$ on a :
    \begin{enumerate}
  ...
  \begin{thm}
    ...
    \begin{array}{l}
      \text{il existe $n \in \mathds{N}$ tel que $a$}\\
  ...
  \begin{defn}
    ...
    \begin{enumerate}
      \item si $(n_1, \ldots, n_p) \in A$ alors $\mathcal{P}_0 \vdash F(\repr{n_1}, \ldots, \repr{n_p})$ ;
      \item si $(n_1, \ldots, n_p) \not\in A$ alors $\mathcal{P}_0 \vdash \lnot F(\repr{n_1}, \ldots, \repr{n_p})$.
    \end{enumerate}
  ...
  \begin{exm}[Les briques de base des fonctions récursives]~\\[-\baselineskip]
    \begin{itemize}
      \item La fonction nulle $f : \mathds{N}\to \mathds{N}, x \mapsto 0$ est représentée par $F(x_0, x_1) := x_0 = \zero$.
      \item Les fonctions constantes $f : \mathds{N}\to \mathds{N}, x \mapsto n$ sont représentées par $F(x_0, x_1) := x_0 = \repr{n}$, où $n \in \mathds{N}$.
      \item Les projections $\pi^i_p : \mathds{N}^p \to \mathds{N}, (x_1, \ldots, x_p) \mapsto x_i$ sont représentées par $F(x_0, x_1, \ldots, x_p) := x_0 = x_i$.
      \item La fonction successeur $f : \mathds{N}\to \mathds{N}, x \mapsto x + 1$ est représentée par $F(x_0, x_1) := x_0 = \succ x_1$.
      \item L'addition $f : \mathds{N}^2 \to \mathds{N}, (x,y) \mapsto x+y$ est représentée par $F(x_0, x_1, x_2) := x_0 = x_1 \oplus x_2$.
      \item La multiplication $f : \mathds{N}^2 \to \mathds{N}, (x,y) \mapsto x\times y$ est représentée par $F(x_0, x_1, x_2) := x_0 = x_1 \otimes x_2$.
    \end{itemize}
  \end{exm}
  ...
  \begin{description}
    \item[Récurrence.]
      ...
      On définit une fonction partielle $f$ par :
      \begin{itemize}
    ...
    \item[Schéma $\mu$.]
      ...
      Soit \[
        g(x_1, \ldots, x_p) := \mu y.\: (f(y, x_1, \ldots, x_p) = 0)
      .\]
      Elle est définie en $(x_1, \ldots, x_p)$ si et seulement s'il existe $y$ tel que $f(y, x_1, \ldots, x_p) = 0$ et tous les $f(x, x_1, \ldots, x_p)$ sont définies pour $x \le y$.
      Dans ce cas, $g(x_1, \ldots, x_p)$ est le plus petit $y$ tel que $f(y, x_1, \ldots, x_p) = 0$.
  \end{description}
  ...
  \begin{rmk}
    \begin{itemize}
      ...
      \item Une fonction récursive peut se fabriquer avec un seul schéma $\mu$ à la fin (\textit{c.f.} cours de FDI).
    \end{itemize}
  \end{rmk}
  ...
  \begin{prv}
    ...
    \begin{center}
      \footnotesize
      $\displaystyle H(x_0, \ldots, x_p) := \exists y_0\: \cdots \: \exists y_n \: \big(
    ...
    \begin{itemize}
      \item $\mathcal{P}_0 \vdash \forall y \: \big(F_i(y_1, \repr{n_1}, \ldots, \repr{n_p}) \leftrightarrow y = \repr{f_i(n_1, \ldots, n_p)}\big)$ ;
      \item $\mathcal{P}_0 \vdash \forall y \: \big( G(y_1, \repr{n_1}, \ldots, \repr{n_n}) \leftrightarrow y = \repr{g(n_1, \ldots, n_n)}\big)$.
    \end{itemize}
    Dans tout modèle $\mathcal{M}$ de $\mathcal{P}_0$, pour tout $y \in |\mathcal{M}|$, et tous $n_1, \ldots, n_p \in \mathds{N}$, $H(y, n_1, \ldots, n_p)$ est vraie ssi il existe $y_1, \ldots, y_n$ dans $|\mathcal{M}|$ tels que pour tout $i$, $F_i(y_i, x_1, \ldots, x_p)$ est vrai et $G(y, y_1, \ldots, y_n)$ est vrai.
    Donc, avec les hypothèses précédents, on a $H(y, n_1, \ldots, n_p)$ ssi il existe $y_1, \ldots, y_n$ dans $|\mathcal{M}|$ tels que pour tout $i$, $y_i = f_i(n_1, \ldots, n_p)$, et $y = g(y_1, \ldots, y_p)$, i.e. ssi \[
  ...
  \begin{prv}
    Soit $g : \mathds{N}^{p+1} \to \mathds{N}$ une fonction représentable totale, et soit $f : \mathds{N}^p \to \mathds{N}$ définie par \[
      f(x_1, \ldots, x_p) := \mu x_0.\:\big( g(x_0, \ldots, x_p) = \repr 0 \big)
    .\]
    Supposons $f$ totale et montrons qu'elle est représentable.
    Soit $G (y, x_0, \ldots, x_p)$ qui représente $g$.
    Alors, pour tous $n_1, \ldots, n_p$ on a 
    \[
      \mathcal{P}_0 \vdash \forall y \: \big(G(y, \repr{n_1}, \ldots, \repr{n_p}) \leftrightarrow y = \repr{g(n_1, \ldots, n_p)}\big)
    .\]
    Considérons la formule

    \begin{center}
      \small
      $F(y, x_1, \ldots, x_p) := G(0, y, x_1, \ldots, x_p)  \land \forall z < y, \lnot G(0, z, x_1, \ldots, x_p)$,
    \end{center}

    où l'on note $\forall z < y, \: H$ pour  $\forall z \: ((\exists h \: y = \succ (z \oplus h)) \to H)$.
    Montrons que $F$ représente $f$.
    Soit $\mathcal{M}$ un modèle de $\mathcal{P}_0$.
    Soient $ n_1, \ldots, n_p$ des entiers et $y \in |\mathcal{M}|$.
    On a $F(y, n_1, \ldots, n_p)$ ssi $G(0, y, n_1, \ldots, n_p)$ est vrai et, pour tout $z < y$, $\lnot G(0, z, n_1, \ldots, n_p)$ est vrai.
    ...
    \begin{itemize}
      ...
      \item Sinon $y > b$. Rappelons que $g(b, x_1, \ldots, x_p) = 0$, de sorte que la formule $\forall  z < y, \: \lnot G(0, z, x_1, \ldots, x_p)$ est fausse, et donc $F(y, n_1, \ldots, n_p)$ aussi.
    \end{itemize}
 \end{prv}
 ...
 \begin{prv}
   ...
   On a dans $\mathds{N}$ : $y = f(x_0, \ldots, x_p)$ ssi il existe $z_0, \ldots, z_{x_0}$ tels que 
   \begin{itemize}
 ...
 \begin{prv}
   ...
   On pose $a = m!$, où $m = \max(p, n_0, \ldots, n_p)$.
   Alors, on pose $a_i := a (i+1) + 1$ pour tout $0 \le i \le p$.
   Les $a_i$ sont bien deux à deux premiers.
   En effet, pour $j > i$, si $a_i$ et $a_j$ ont un facteur premier commun $c$ , alors $c  \mid (a_i - a_j)$ donc $c  \mid a (j-i)$. Comme $c$ est premier, on a $c \mid a$ ou $c \mid j-i$. Dans les deux cas, puisque $j - i \le p \le m$ on a $c \mid a$. Or $c$ divise aussi $a_i$, donc $c$ divise $1$, ce qui est impossible.
   Ainsi, avec $b$ tel que $b \equiv n_i \pmod {a_i}$ pour tout $0 \le i \le p$ et sachant que pour tout $0 \le i \le p$, $n_i < a_i$,
   on définit $\beta(i, a, b)$ comme le reste de la division de $b$ par $a(i+1) + 1$.
   La fonction $\beta$ est représentée par  \[
   B(x, i, a, b) := \exists q \: \big(b = q \otimes \succ (a \otimes \succ i) \oplus x \land x < \succ(a \otimes \succ i)\big)
   .\]
   On considère
   \[
   B'(x_0, x_1, x_2, x_3) := B(x_0, x_1, x_2, x_3) \land \forall x_4 < x_0, \:\lnot B(x_4, x_1, x_2, x_3)
   .\] 
   Cette dernière formule représente aussi $\beta$ mais implique aussi que $x_0$ est un entier standard.
   %QV : Ceci mérite une justification.
   %QV : Il faut aussi expliquer pourquoi $\beta$ est primitive récursive.
   %HS : Je suis d'accord, c'est une histoire de "schéma µ borné" qui peut se faire en primitif récursif
 \end{prv}
 ...
 \begin{prv}
   ...
   \begin{itemize}
     ...
     \item $y = z_{x_0}$
   \end{itemize}
   c'est-à-dire ssi la formule
   \begin{align*}
     F := \quad \exists a \: \exists b \: \big[ &\\
    & (\exists z_0 \: B'(z_0, \repr{0}, a, b) \land G(z_0, x_1, \ldots, x_p)) \\
       {}\land {} & \forall i < x_0, \: \exists z \: \exists z' \: \left(\begin{array}{rl}
      &B'(z, i, a, b)\\
      \land &B'(z', \succ i, a, b)\\
   ...
   \end{align*}
   est vraie.
   En effet, soient $\mathcal{M} \models \mathcal{P}_0$, et $n_0, \ldots, n_p$ des entiers et $c \in |\mathcal{M}|$.
   \begin{itemize}
 ...
 Maintenant que l'on a transformé les fonctions en formules, faisons l'opposé.
 Notre but est de montrer le théorème suivant :
 toute théorie consistante contenant $\mathcal{P}_0$ est indécidable.
 ...
 \begin{lem}[Récursion]
   ...
   \begin{itemize}
     \item $k_1, \ldots, k_n : \mathds{N} \to \mathds{N}$ telles que $\forall y \neq 0, \forall i$, $k_i(y) < y$ ;
   ...
   et pour tout $y \in \mathds{N}^*$
   \begin{align*}
     f(y, x_1, \ldots, x_p) := h(y, u_1, \ldots, u_n, x_1, \ldots, x_p)
   ,\end{align*}
   où l'on note $u_i := f(k_i(y), x_1, \ldots, x_p)$ pour tout  $i \in \llbracket 1, n\rrbracket$.
   \qed
 \end{lem}

 \begin{lem}[Définition par cas]
   Soient $P_1, \ldots, P_n$ des parties de $\mathds{N}^m$ récursives primitives (\textit{resp}. récursives) deux à deux disjointes et $f_1, \ldots, f_{n+1} : \mathds{N}^m \to \mathds{N}$ des fonctions récursives primitives (\textit{resp}. récursives). Alors la fonction suivante est récursive primitive (\textit{resp}. récursive) :
   \[
 ...
 \begin{lem}[Définition par cas et récursion]
   ...
   \end{itemize}
   des fonctions récursives primitives (\textit{resp}. récursives) et $P_1, \ldots, P_n$ des parties de $\mathds{N}^p$ deux à deux disjointes et récursives primitives (\textit{resp.} récursives). Supposons que pour tous $1 \le i \le m$ et $y \in \mathds{N}^*$, $k_i(y) < y$. Alors la fonction $f$ définie par les équations ci-dessous est récursive primitive (resp. récursive) :
   \[f(0, x_1, \ldots, x_p) := g(x_1, \ldots, x_p)\]
   et pour tout $y \in \mathds{N}^*$
    \[
   f(y, x_1, \ldots, x_p) := \begin{cases}
     f_1(y, u_1, \ldots, u_m, x_1, \ldots, x_p) & \text{si } P_1(x_1, \ldots, x_p)\\
     f_2(y, u_1, \ldots, u_m, x_1, \ldots, x_p) & \text{si } P_2(x_1, \ldots, x_p)\\
     \quad\quad\quad\quad \vdots\\
     f_n(y, u_1, \ldots, u_m, x_1, \ldots, x_p) & \text{si } P_n(x_1, \ldots, x_p)\\
     f_{n+1}(x_1, \ldots, x_p) & \text{sinon},
   \end{cases}
   \] 
   où l'on note $u_i := f(k_i(y), x_1, \ldots, x_p)$ pour tout  $i \in \llbracket 1, m\rrbracket$.
   \qed
 \end{lem}
 ...
 \begin{defn}
   ...
   \begin{itemize}
     \item si $t = \repr{0}$ alors  $\sharp t := \alpha_3(0, 0, 0)$ ;
     \item si $t = x_n$ alors  $\sharp t := \alpha_3(n+1, 0, 0)$ ;
     \item si $t = \succ t_1$ alors $\sharp t := \alpha_3(\sharp t_1, 0, 1)$ ;
     \item si $t = t_1 \oplus t_2$ alors $\sharp t := \alpha_3(\sharp t_1, \sharp t_2, 2)$ ;
     \item si $t = t_1 \otimes t_2$ alors $\sharp t := \alpha_3(\sharp t_1, \sharp t_2, 3)$.
   \end{itemize}
 \end{defn}
 ...
 \begin{prv}
   Montrons que la fonction indicatrice $T$ de $\mathrm{Term}$ est récursive primitive.
 ...
 \begin{lem}
   ...
   \footnotesize
   \begin{itemize}
     \item $\theta_0 := \mleft\{\,(\sharp t, n) \;\middle|\; \begin{array}{l} \text{$t$ est un terme et $x_n$ n'a}\\ \text{pas d'occurrence dans $t$} \end{array} \,\mright\}$ ;
     \item $\theta_1 := \mleft\{\,(\sharp t, n) \;\middle|\; \begin{array}{l} \text{$t$ est un terme et $x_n$ a}\\ \text{une occurrence dans $t$} \end{array} \,\mright\}$ ;
     \item $\phi_0 := \mleft\{\,(\sharp F, n) \;\middle|\; \begin{array}{l} \text{$F$ est une formule et $x_n$ n'a}\\ \text{pas d'occurrence dans $F$} \end{array} \,\mright\}$ ;
     \item $\phi_1 := \mleft\{\,(\sharp F, n) \;\middle|\; \begin{array}{l} \text{$F$ est une formule et $x_n$ n'a}\\ \text{pas d'occurrence libre dans $F$} \end{array} \,\mright\}$ ;
     \item $\phi_2 := \mleft\{\,(\sharp F, n) \;\middle|\; \begin{array}{l} \text{$F$ est une formule et $x_n$ n'a}\\ \text{pas d'occurrence liée dans $F$} \end{array} \,\mright\}$ ;
     \item $\phi_3 := \mleft\{\,(\sharp F, n) \;\middle|\; \begin{array}{l} \text{$F$ est une formule et $x_n$ a}\\ \text{une occurrence libre dans $F$} \end{array} \,\mright\}$ ;
     \item $\phi_4 := \mleft\{\,(\sharp F, n) \;\middle|\; \begin{array}{l} \text{$F$ est une formule et $x_n$ a}\\ \text{une occurrence liée dans $F$} \end{array} \,\mright\}$ ;
     \item $\phi_5 := \mleft\{\,\sharp F \;\middle|\; \begin{array}{l} \text{$F$ est une formule close} \end{array} \,\mright\}$.
   \end{itemize}
 \end{lem}
 \begin{prv}
   On montre le résultat pour $\theta_0$ (le reste en exercice).
   On définit la fonction indicatrice de $\theta_0$, notée  $g_0(x, y)$, par (en utilisant le lemme de définition par cas et récursion) :
    \begin{itemize}
     \item si $\beta_3^3(x) = \beta_3^2(x) = 0$ et  $\beta_3^1(x) \neq y + 1$ alors $g_0(x, y) := 1$ ;
     \item si $\beta_3^3(x) = 1$ et $\beta_3^2(x) = 0$ alors  $g_0(x,y) := g_0(\beta_3^1(x), y)$ ;
 ...
 \begin{lem}[Substitutions]
   Il existe des fonctions récursives primitives $\mathrm{Subst_t}$ et $\mathrm{Subst_f}$ à trois variables telles que, si $t$ et $u$ sont des termes, et si $F$ est une formule, alors pour tout entier $n$, 
   \begin{itemize}
 ...
 \begin{prv}
   ...
   \begin{itemize}
     \item si $\beta_3^3(x) = 0$  alors
       ...
       \end{itemize}
     \item si $\beta_3^3(x) = 1$ alors \[
     \mathrm{Subst_t}(n, y, x) := \alpha_3(\mathrm{Subst_t}(n, y, \beta_3^1(x)), 0, 1) \;;
     \] 
     \item sinon 
       \begin{center}
         \footnotesize
         $\mathrm{Subst_t}(n, y, x) := \alpha_3(\mathrm{Subst_t}(n, y, \beta_3^1(x)), \mathrm{Subst_t}(n, y, \beta_3^2(x)), \beta_3^3(x))$ ;
       \end{center}
   \end{itemize}

   Puis, on définit $\mathrm{Subst_f}$ par, pour $(n, y, x)$,
   \tiny
   \begin{itemize}
     \item si $\beta_3^3(x) = 0$ alors $\mathrm{Subst_f}(n, y, x) = \alpha_3(\mathrm{Subst_t}(n, y, \beta_3^1(x))), \mathrm{Subst_t}(n, y, \beta_3^2(x), 0)$ ;
     ...
     \item si $\beta_3^3(x) = 5 \text{ ou } 6$ alors
       \begin{itemize}
         \item si $\beta_3^2(x) = n$, $x_n$ est liée dans $F$ donc $\mathrm{Subst_f}(n, y, x) := x$ ;
         \item sinon $\mathrm{Subst_f}(n, y, x) := \alpha_3(\mathrm{Subst_f}(n, y, \beta_3^1(x)), \beta_3^2(x), \beta_3^3(x))$ ;
       \end{itemize}
     \item sinon $\mathrm{Subst_f}(n, y, x) := x$.
   \end{itemize}
 \end{prv}
 ...
 \begin{lem}
   La substitution d'une formule dans un contexte est récursive primitive.
 ...
 \begin{defn}
   Un ensemble $A$ de formules est un ensemble d'\textit{axiomes} d'une théorie $T$ si $A \vdash T$ et $T \vdash A$.
 \end{defn}

 \begin{defn}
   Une théorie $T$ sur $\mathcal{L}_0$ a un ensemble d'axiomes $\mathrm{Ax}_T$ récursif si l'ensemble des numéros des formules de $\mathrm{Ax}_T$ est récursif.
 \end{defn}
 \begin{rmk}
 ...
 \begin{prop}
   Si une théorie $T$ a un ensemble d'axiomes $\mathrm{Ax}_T$ récursif alors l'ensemble \[
   \mathrm{Dem}_T = \mleft\{\, (\sharp D, \sharp F) \;\middle|\; \text{$D$ est une preuve de $F$ dans $T$ avec $\mathrm{Ax}_T$}\,\mright\} 
   \] 
   est récursif.
 \end{prop}
 \begin{prv}
   L'idée de la preuve est la suivante : pour $(x, y)$,
   \begin{enumerate}
     \item vérifier que $x$ est le code d'une preuve $D$ et $y$ le code d'une formule $F$ ;
 ...
 \begin{exm}[Quelques théories décidables]
   \begin{itemize}
     \item La théorie des ordres denses sans extrémités (la théorie linéaire des rationnels) est une théorie décidable.
     \item La théorie des corps réels clos (\textit{théorème de Tarski}) est une théorie décidable.
     \item L'arithmétique de Presburger (la théorie linéaire des entiers) est une théorie décidable.
     \item Pour chaque $p$, la théorie des corps algébriquement clos de caractéristique $p$ est une théorie décidable.
   \end{itemize}
 \end{exm}
 ...
 \begin{prv}
   ...
   Si $T$ est récursif, alors $T_0$ aussi et $\mathcal{P}_0$ est décidable. \textit{\textbf{absurde}}.
 \end{prv}

 \section{Théorèmes d'incomplétude de Gödel}

 \begin{thm}[Premier théorème d'incomplétude de Gödel]
   Soit $T$ une théorie qui a un ensemble d'axiomes récursifs, qui est consistante, et qui contient $\mathcal{P}_0$. Alors, $T$ n'est pas axiome-complète.
 \end{thm}
 \begin{prv}
   Une théorie qui a un ensemble d'axiomes récursifs et qui est complète est décidable.

   En effet, pour $F$ une formule, comment déterminer (algorithmiquement) si $T \vdash F$ ?
 ...
 \begin{prop}
   \begin{itemize}
     \item Ces ensembles sont récursifs donc représentés par deux formules $F_T$ et $F_{\mathcal{P}_0}$.
     \item La fonction $\mathrm{neg} : \mathds{N} \to \mathds{N}, \sharp F \mapsto \sharp (\lnot F) = \alpha_3(\sharp F, 0, 1)$ est récursive et représentée par une formule $F_{\mathrm{neg}}(x_0, x_1)$ :
       \[
 ...
 \begin{lem}
   ...
   \begin{enumerate}
     ...
     \item si $F$ est une formule close de $\Sigma$ alors \[
       \mathcal{P} \vdash (F \to \exists x_1 \: F_{\mathcal{P}_0} (x_1, \sharp F))
       .\]
   \end{enumerate}
 \end{lem}
 \begin{prv}
   \begin{enumerate}
     ...
     \item On va le montrer pour une théorie $\mathcal{P}_1$ contenant $\mathcal{P}_0$ et conséquence de $\mathcal{P}$ mais, \textit{a priori}, plus faible que $\mathcal{P}$.
       ...
       On le montre avec la proposition suivante.
   \end{enumerate}
 \end{prv}
 ...
 \begin{prv}
   \begin{itemize}
     ...
     \item Si $\mathds{N} \models F$, il faut montrer que $F$ a une preuve dans $\mathcal{P}_0$, \textit{i.e.} que pour tout modèle $\mathcal{M} \models \mathcal{P}_0$, on a $\mathcal{M} \models F$ \textit{i.e.} que dans tout extension finale $\mathcal{M}$ de $\mathds{N}$ alors $\mathcal{M} \models F$, pour cela il suffit de montrer le lemme suivant.
   \end{itemize}
 \end{prv}
 ...
 \begin{prv}
   ...
   Les cas intéressants sont les quantificateurs. Pour "$\exists$", on applique que $|\mathcal{N}| \subseteq |\mathcal{M}|$.
   Pour le "$\forall$" \textit{borné}, on utilise la finalité de l'extension $\mathcal{M}$ de $\mathcal{N}$.
 \end{prv}
 ...
 \begin{lem}[Cœur du 2nd théorème d'incomplétude]
   Soit $T$ une théorie consistante, axiome-récursive, et contenant $\mathcal{P}$.
   Alors,~$T \not\vdash \mathrm{Coh}(T)$.
 \end{lem}
 \begin{prv}
   \begin{itemize}
     \item Soit $g : \mathds{N} \to \mathds{N}$ définie par, pour tout $n$, s'il existe une formule $F(x_0)$ telle que $n = \sharp F(x_0)$, alors on pose $g(n) = \sharp F(\repr{n})$, sinon $g(n) = 0$.
     ...
     \item Par définition, $\varepsilon(\repr{a})$ est  $\exists x_1 \: \exists x_2 \: (F_T(x_1, x_2) \land G(x_2, \repr{a}))$.
       "Il existe une preuve du fait qu'il n'existe pas de preuve de nous-même".
       Dans $\mathcal{P}_0$, avec le point précédent, $\varepsilon(\repr{a})$ est équivalent à $\exists x_1 \: F_T(x_1, \repr{b})$ ($\star$).
   ...
   \begin{itemize}
     \item Prouvons que $T \vdash \mathrm{Coh}(T) \to \lnot \varepsilon(\repr a)$.
       Il suffit de montrer que $\mathcal{P} \vdash \varepsilon(\repr{a}) \to \lnot \mathrm{Coh}(T)$.
       Soit $T_1 := \mathcal{P} \cup \{\varepsilon(\repr{a})\}$.
       Alors d'après ($\star$), $T_1 \vdash \exists v_1 \: F_T(v_1, \repr b)$ et $b = \sharp (\lnot \varepsilon(\repr{a}))$.
       On a donc une preuve de $\varepsilon(\repr{a})$ et une preuve de  $\lnot \varepsilon(\repr a)$, donc de $\lnot \mathrm{Coh}(T)$ en donnant (l'encodage de) ces deux preuves.
       %QV : Comment obtiens-tu cette preuve ? Où utilises-tu la forme particulière de Coh ?
       %HS : Problème d'entiers non-standards ? J'ai corrigé pour la remarque sur Coh
     \item On va montrer que $T \vdash \lnot \varepsilon(\repr{a})$ mène à un paradoxe.
       ...
       D'où, avec ($\star$), $\mathcal{P}_0 \vdash \varepsilon(\repr{a})$, ce qui est impossible car $T$ est consistante.
