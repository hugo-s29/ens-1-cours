...
\begin{document}
  ...
  \begin{rmk}
    \begin{itemize}
      ...
      \item On aura toujours, sauf si précisé autrement, les relations "$=$" d'arité $2$, et "$\bot$" d'arité $0$.
  ...
  \begin{exm}
    ...
    \begin{itemize}
      ...
      \item trois fonctions $\cap$, $\cup$ et $\square^\mathsf{c}$,
        ...
        %HS : Pour l'instant, c'est juste de la notation, on n'a pas encore associé une sémantique. Après, à voir si c'est utile, je n'ai pas l'impression que -ᶜ, ∪, ∩ et ⊆ ne soient utiles dans le chap 4. Potentiellement, c'est juste là pour donner plus d'exemples.
  ...
  \begin{defn}
    Les formules sont des mots sur l'alphabet \[ \mathcal{L} \cup \mathcal{V} \cup \{\verb|(|, \verb|)|, \verb|,|, \exists, \forall, \land, \lor, \lnot, \to \}. \]

    L'ensemble des \textit{formules} $\mathcal{F}$ du langage $\mathcal{L}$ est défini par 
    \begin{itemize}
      \item on pose $\mathcal{F}_0$ l'ensemble des \textit{formules atomiques}, c'est-à-dire l'ensemble des formules de la forme $R(t_1, \ldots, t_n)$ où $R$ est un symbole de relation d'arité $n$ et $t_1, \ldots, t_n$ des termes ;
      \item on pose, pour tout $k \in \mathds{N}$,
        \[
          \mathcal{F}_{k+1} := \mathcal{F}_k \cup \mleft\{\,
            \begin{array}{c}
              (\lnot F)\\
              (F \to G)\\
              (F \lor G)\\
  ...
  \begin{exm}
    \begin{itemize}
      ...
      \item Dans $\mathcal{L}_\mathrm{ens}$, la loi de De Morgan s'écrit \[
          ...
          %HS : cf. mon commentaire précédent sur les complémentaires.
        .\]
    \end{itemize}
  \end{exm}
  ...
  \begin{rmk}[Conventions d'écriture.]
    ...
    \begin{enumerate}
      ...
      \item les symboles $\lnot, \exists, \forall$ ;
        ...
        %HS : Je suis totalement d'accord, j'ai dû corriger ça plusieurs fois, j'avais des quantificateurs qui prennaient toutes la formule, mais ça ne correspond pas à la convention donnée
        %HS : Je ne pense pas que ça soit une "bonne" idée de changer ça dans les notes de cours uniquement, sinon les formules au tableau/en exam pourront être interprétées différemment.
  ...
  \begin{defn}
    \begin{itemize}
      ...
      \item La racine de l'arbre est 
        \begin{itemize}
          \item "$F$" si la formule $F$ est atomique ;
            %QV : C'est un peu bizarre, pourquoi le seul noeud de l'arbre ne serait-il pas qualifié de racine ?
            %HS : Il l'est, mais ce n'est pas très net... dans ce cas on aurait S(formule atomique F) = ∅ j'ai l'impression
            %HS : Je change la définition
  ...
  \begin{defn}
    ...
    %HS : Je suis d'accord que le mot "prouvable" est bizarre... dérivable fait plus penser à dériver un séquent, donc je ne sais pas s'il y a mieux. (ou prouvable pour le cas particulier de la théorie vide.)
  \end{defn}
    ...
    \begin{description}
      ...
      \item[Quantificateur universel.]
        ...
        \begin{prooftree}
          ...
          \infer 1[\forall_\mathsf{e}]{\Gamma \vdash A[x := t]}
          %QV : C'est déjà dans la définition de substitution.
          %HS : J'ai rajouté la remarque/rappel en dessous
        \end{prooftree}
        \]
        Rappelons que pour la substitution $A[x := t]$, on devra potentiellement renommer des variable liées de $A$ qui apparaissent dans $t$.
  ...
  \begin{rmk}
    ...
    \begin{prooftree}
      ...
      \infer 0[\mathsf{ax}]{x = 0 \vdash x = 0}
  ...
  \begin{defn}
    ...
    On appelle \textit{interprétation} (ou \textit{structure}, ou \textit{modèle}) du langage $\mathcal{L}$ l'ensemble $\mathcal{M}$ des données suivantes :
    \begin{itemize}
  ...
  \begin{defn}
    ...
    \begin{itemize}
      ...
      \item La \textit{valeur} d'un terme $t$ dans l'environnement $e$, notée $\Val_{\mathcal{M}}(t,e)$, est définie par induction sur l'ensemble des termes de la façon suivante :
        ...
        \begin{itemize}
          \item si $c$ est une constante, $\Val_{\mathcal{M}}(c, e) := c_{\mathcal{M}}$ ;
          \item si $x$ est une variable $\Val_{\mathcal{M}}(x,e) = e(x)$ ;
          \item si $f$ est une fonction $n$-aire,
            \[
            \Val_{\mathcal{M}}(f(t_1, \ldots, t_n), e) = f_{\mathcal{M}}(\Val_{\mathcal{M}}(t_1, e), \ldots, \Val_{\mathcal{M}}(t_n, e))
            .\] 
        \end{itemize}
    \end{itemize}
  \end{defn}
  ...
  \begin{nota}
    \begin{itemize}
      ...
      \item À la place de  $\Val_{\mathcal{M}}(t,e)$  quand $x_1, \ldots, x_n$ sont les variables de $t$ et $e(x_1) = a_1, \ldots, e(x_n) = a_n$, on note $t[a_1, \ldots, a_n]$ ou aussi~$t[x_1 := a_1, \ldots, x_n := a_n]$.
        ...
        %HS : La notation t[x_1 := ..., ...] peut faire penser à une substitution (même notation qu'une substitution) d'une variable par un terme
        %     ou à la notation F(x_1, ...) pour les formules.
        %HS : Je suis d'accord que le nom "terme à paramètre", c'est pas très clair.
    \end{itemize}
  \end{nota}
  ...
  \begin{defn}
    ...
    \begin{itemize}
      ...
      \item $\Val_{\mathcal{M}}(F \land G, e) = \min(\Val_{\mathcal{M}}(F, e), \Val_{\mathcal{M}}(G, e))$ ;
  ...
  \begin{rmk}
    Dans le cas des formules closes, pour obtenir la valeur de $F$, on doit quand même utiliser un environnement lorsqu'on doit traiter un $\forall $ ou un $\exists$.
    %QV : Je ne comprends pas ce que tu veux dire par là.
    %HS : J'espère que c'est plus clair
  \end{rmk}
  ...
  \begin{nota}
    ...
    %HS : Même réponse que pour les « termes à paramètres ».
  \end{nota}
  ...
  \begin{exm}
    ...
    \end{itemize}
    Aucune des deux n'est un théorème ($F$ n'est vrai que dans les groupes, et c'est même la définition de groupe, et $G$ est vraie en particulier dans les monoïdes (unicité du neutre à droite)\footnote{ Et, il existe des modèles qui ne vérifient pas $G$, il suffit par exemple de définir $x *_{\mathcal{M}} y := x$ où $\mathcal{M}$ a au moins deux éléments}), mais $F \to G$ est un théorème logique.
    ...
    %HS : Fixed, j'espère que c'est plus clair.
  \end{exm}
  ...
  \begin{exm}
    ...
    \] est parfois vraie, parfois fausse, cela dépend du groupe considéré.\footnote{On applique ici le théorème de correction, vu plus tard.}
  ...
  \begin{exm}
    ...
    %HS : Je suis d'accord. Avec un peu de recul sur le cours, je ne comprends pas trop la distinction entre les "deux notions de complet" qu'on a eu en cours
    %HS : Je pense que c'est là uniquement pour ne pas qu'on confonde le théorème de complétude de Gödel et l'incomplétude de Gödel.
  \end{exm}
  ...
  \begin{rmk}[\textbf{Attention !}]
    ...
    \begin{itemize}
      ...
      \item Dans le sens $\textsl{théorie} \models \textsl{formule}$, on dit qu'une formule est vraie dans tous les modèles de la théorie, c'est un sens plus inhabituel.
        ...
        %HS : À transmettre à Natacha PORTIER, c'est elle qui le dit
    \end{itemize}
  \end{rmk}

  \subsection{Preuve du théorème de correction.}

  Montrons la version 1 du théorème de correction.
  ...
  \begin{prop}
    ...
    %HS : Oui (modulo un ∀ qui bouge), je retire l'exercice
  \end{prop}
  ...
  \begin{prv}
    ...
    On termine la preuve en montrant le lemme (plus fort que la proposition) ci-dessous.
