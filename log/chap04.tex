...
\begin{document}
  ...
  Pour éviter ce paradoxe, on choisit donc de ne pas faire de $\mathcal{U}$ un ensemble.
  %QV : Ça ne ferait pas vraiment sens, $\mathcal{U}$ est l'ensembles des éléments du modèle, pas un élément du modèle lui-même. Autrement dit, dans $x \in \mathcal{U}$, le $\in$ est l'appartenance habituelle, pas le symbole de relation du langage.
  %HS : Je suis d'accord, mais je pense que ça va, ce n'est que le texte introductif.
  ...
  \begin{nota}
    ...
    \begin{itemize}
      ...
      \item $t \subseteq a$ pour $\forall x \: (x \in t \to x \in a)$.
    \end{itemize}
  \end{nota}
  ...
  \begin{description}
    \item[ZF\,4'.\label{ZF4'}]
      %QV : ZF4' avant ZF4 est aussi un peu bizarre.
      %HS : Oui, mais ZF4 est plus simple à comprendre 
      \textit{Schéma de compréhension} :
      pour tout $(n+1)$-uplet d'ensembles $(v_1, \ldots, v_{n+1})$, et toute formule de la forme $\varphi(y, v_1, \ldots, v_n)$, on a l'ensemble
      $x = \mleft\{\,y \in v_{n+1} \;\middle|\; \varphi(y, v_1, \ldots, v_n)\,\mright\}$
      %QV : Les deux premières lignes débordent, mais je n'ai pas le courage de réparer.
      %HS : C'est réparé :)
      \[
      \forall v_1\: \ldots\: \forall v_{n+1} \: \exists x \: \forall y \: \big(y \in x \leftrightarrow (y \in v_{n+1} \land \varphi(y, v_1, \ldots, v_n))\big)
      .\] 
  ...
  \begin{rmk}
    ...
    Et, on ne peut pas avoir de paradoxe avec $b := \{z \in E  \mid z \not\in z\}$ grâce à l'ajout de la condition "$z \in E$".
  \end{rmk}

  \begin{defn}
    Une \textit{relation fonctionnelle} en $w_0$ est une formule $\varphi(w_0, w_1, a_1, \ldots, a_n)$ \textit{à paramètres} (où les $a_i$ sont dans $\mathcal{U}$) telle que

    \fitbox{$\mathcal{U} \models \forall w_0 \: \forall w_1 \: \forall w_2 \: \big(\varphi(w_0, w_1, a_1, \ldots, a_n) \land \varphi(w_0, w_2, a_1, \ldots, a_n)\to w_1 = w_2\big)$.}
  \end{defn}
  ...
  \begin{description}
    \item[ZF\,4.\label{ZF4}]
      ...
      Pour tout $n$-uplet $\bar{a}$, si la formule à paramètres $\varphi(w_0, w_1, \bar{a})$ définit une relation fonctionnelle $f_{\bar{a}}$ en $w_0$ et si $a_0$ est un ensemble alors la collection des images par $f_{\bar{a}}$ des éléments de $a_0$ est un ensemble noté $\{ f_{\bar{a}}(x) \mid x \in a_0\}$.
  \end{description}
...
\big(\big(
      ...
      \big)\\
      \vertical\to\\
      \exists x \: \forall y (y \in x \leftrightarrow \exists w_0 \: w_0 \in a_0 \land \varphi(w_0, y, v_1, \ldots, v_n))\big)
  .\end{gather*}
  ...
  \begin{prv}
    \begin{itemize}
      ...
      \item \textit{Existence}.
        ...
        Par \zf 4 (avec la formule $\varphi$ et l'ensemble  $a_0 := x$), il existe un ensemble $a_{n+1}$ qui est vide.
    \end{itemize}
  \end{prv}
  ...
  \begin{prv}
    ...
    Comme  $\varphi$ est bien une relation fonctionnelle, on peut appliquer \zf 4 avec $a_0 = \{\emptyset, \{\emptyset\}\}$ et on obtient $\{a, b\}$.
  \end{prv}
  ...
  \begin{prv}
    ...
    \mathcal{U} \models \forall v_1 \: \cdots \: \forall v_{n+1} \: \exists x \: \forall y \: \big(
      y \in x \leftrightarrow (y \in v_{n+1} \land \varphi(y, \underbrace{v_1, \ldots, v_n}_{\bar{v}}))
    \big)
    .\]
  ...
  \begin{prv}
    ...
    On applique \zf{4'} dans l'ensemble ambiant  $\wp(\wp(v_1 \cup v_2))$, on définit le produit par compréhension à l'aide de la formule 
    \[
  ...
  \begin{defn}
    Une \textit{fonction} (sous-entendue \textit{totale}) d'un ensemble $a$ dans un ensemble $b$ est un sous-ensemble de $a \times b$ qui vérifie la propriété
    %HS : Rien n'empêche que l'ensemble vide ne soit pas dans f par exemple, la condition f \subseteq a \times b m'est justifiée
    %HS : On peut dire qu'on ignore les éléments qui sont pas (x, y) mais c'est bof (et tu perds l'extentionalité au passage)
    \[
    \varphi(f, a, b) := 
    \begin{pmatrix} 
      f \subseteq a \times b\\
      \land\\
      \forall x \: \forall  y \: \forall y' \: \big((x, y )\in f \land (x, y') \in f  \to y = y'\big)\\
      \land\\
      \forall x \: \big(x \in a \to \exists y \: \big(y \in b \land (x,y) \in f\big)\big)
    \end{pmatrix}
    .\]
  ...
  \begin{nota}
    Pour $f \subseteq a \times b$ une fonction et $x \in a$, on note $f(x)$ l'unique élément $y \in b$ tel que $(x, y) \in f$.
  \end{nota}
  ...
  \begin{nota}[Réunion indexée]
    Soit $a$ une famille d'ensemble indexée par l'ensemble $I$, \textit{i.e.} une fonction de domaine $I$.
    Pour $i \in I$, on note $a_i$ au lieu de $a(i)$.
  \end{nota}
  ...
  \end{prop}
  \begin{prv}
    On pose $c := \bigcup_{i \in  I} a_i$ qui est un ensemble par la proposition~\ref{prop:union-is-set}.
    On considère \[
  ...
  \begin{prop}
    Si $I$ est un ensemble et $a$ est une fonction de domaine $I$ alors $\prod_{i \in I} a_i $ est un ensemble.
  \end{prop}
  \begin{prv}
  ...
  \phantomsection
  \begin{description}
    \item[ZF\,5\label{ZF5}] \textit{Axiome de l'infini} : il existe un ensemble ayant une infinité d'éléments
      \[
  ...
  Ainsi, on a $n  =\{0, 1, \ldots, n-1\}$. C'est la représentation des entiers de von Neumann.
  ...
  \begin{rmk}
    ...
    %QV : Wat?
    %HS : Malheureusement c'est vrai parfois...
  \end{rmk}
  ...
  \begin{thm}[Cantor]
    \begin{enumerate}
      \item Soient $A$ et $B$ deux ensembles, supposons qu'il existe des injections $A \to B$ et $B \to A$ alors il existe une bijection $A \to B$.
  ...
  \begin{defn}
    ...
    \begin{enumerate}
      ...
      \item \textit{anti-réflexivité} : $\forall x, x \not< x$.
    \end{enumerate}
  \end{defn}
  ...
  \begin{defn}
    Soit $(A, <)$ un ensemble ordonné. Soient  $a, a' \in A$ et $B \subseteq A$.
    On dit que 
    \begin{itemize}
      \item $a$ est un plus petit élément de $B$ si $a \in B$ et pour tout $b \in B$, $b \ge a$ ;
      ...
      \item $a$ est une borne inférieure de $B$ si $a$ est un plus grand élément de l'ensemble des minorants de $B$ ;
  ...
  \begin{exm}
    ...
    %QV : Ceci est la seule occurrence de « Von Neuman », donc à moins de déjà savoir de quoi tu parles, on ne peut pas comprendre.
    %HS : Fixed
  \end{exm}
  ...
  \begin{prop}
    ...
    \begin{enumerate}
      ...
      \item $\beta \subseteq \alpha$ ssi $\beta = \alpha$ ou  $\beta \in \alpha$ ;
  ...
  \begin{prv}
    \begin{enumerate}
      ...
      \item La relation $\in$ est un bon-ordre sur $\alpha$, soit $\beta$ le plus petit élément.
        Si $\beta \neq \emptyset$ alors il contient au moins un élément $\gamma$ d'où $\gamma < \beta$ et $\gamma \in \alpha$ (par transitivité), \textit{absurde} car $\beta$ est minimal.
      \item Le reste sera vu en TD ou en exercice.
        %QV : Oh, come on... Aussi, TD = exercice, non ?
        %HS : ...
    \end{enumerate}
  \end{prv}
  ...
  \begin{prop}
    \begin{itemize}
      \item Si $\alpha$ et $\beta$ sont des ordinaux et on a~$\alpha \le \beta \le \alpha + 1$ alors $\beta = \alpha$ ou  $\beta = \alpha + 1$.
  ...
  \begin{defn}
    ...
    \begin{itemize}
      ...
      \item Sinon, c'est un ordinal limite.
        %QV : Il me semble que dans le cours de Natacha on exclut 0 des ordinaux limites.
        %HS : Oui ça me semble douteux aussi
    \end{itemize}
  \end{defn}
  ...
  \begin{prop}
    ...
    %QV : Avec ta définition, c'est aussi vrai pour 0.
    \[
  ...
  \begin{prv}
    \begin{enumerate}
      \item Par contraposée, si $\lambda$ n'est pas limite, alors c'est le successeur d'un certain ordinal $\beta$ et donc $\lambda = \beta \cup \{\beta\}$.
        On a \[
  ...
  \begin{thm}[Induction transfinie]
    ...
    \begin{itemize}
      ...
      \item pour tout ordinal non vide $\alpha$ tel que, si tout $\beta < \alpha$ satisfait $\mathcal{P}$, alors $\alpha$ satisfait $\mathcal{P}$ :
        \[
        \forall \alpha, \: (\forall \beta < \alpha, \: \mathcal{P}(\beta)) \implies \mathcal{P}(\alpha) \;
        ;\] 
      %QV : La première hypothèse est inclue dans la deuxième.
      %HS : plus maintenant :)
    \end{itemize}
  ...
  \begin{defn}
    Un ordinal $\alpha$ est \textit{fini} si $\alpha$ et tous ses éléments sont vides ou des successeurs.
  \end{defn}
  ...
  \begin{prv}
    En exercice.
    %QV : N'est pas juste une compréhension avec l'axiome de l'infini ?
    %HS : Oui normalement, mais je pense que c'est surtout un ordinal qui est limite, et en plus le plus petit.
  \end{prv}
  ...
  \begin{prv}
    \begin{itemize}
      ...
      \item Montrons que $f$ est l'identité.
        ...
        %QV : On peut aussi le dire en remarquant que $f$ et $f^{-1}$ sont toutes les deux extensives d'après le premier point.
    \end{itemize}
  \end{prv}
  ...
  \end{thm}
  \begin{prv}
    Cette preuve ressemble à une induction sans en être une.\footnote{Ceci vient du fait qu'une preuve par induction correspond à considérer un contre-exemple minimal, ce qui est exactement ce que l'on fait dans le reste de la preuve.}
    %QV : En fait c'en est vraiment une, une induction n'est rien d'autre qu'une preuve par l'absurde où on considère un contre-exemple minimal.
    %HS : J'aime bien cette explication, je la vole :) (surtout que c'est ce que fait la preuve)
    On aura le droit d'en faire quand on aura le théorème.

    Si l'isomorphisme existe, il est unique grâce au lemme précédent.
    En effet, s'il y en a deux $f$ et $g$ alors $f \circ g^{-1}$ est un automorphisme, donc c'est l'identité.
    ...
    %QV : Il est un peu plus naturel de considérer $S_{< x}$ qui correspond à un ordinal là où $S_{\le x}$ correspond à son successeur.
    ...
    \begin{itemize}
      \item Si $Y$ a un plus grand élément $b$, alors il existe un isomorphisme $f_b : S_{\le b} \to \alpha_b$ (car on a $\mathcal{P}(b)$).
        Or, $S_{\le a} = S_{\le b} \cup \{a\}$ (il faudrait montrer que $Y$ est un segment initial de $X$).
        Et, on construit $f_a : S_{\le a} \to \alpha_b \cup \{\alpha_b\}$ qui est un isomorphisme, donc $a \in Y$, \textit{\textbf{absurde}}.
      \item Si $Y$ n'a pas de plus grand élément, on considère $\alpha := \bigcup_{x \in Y} \alpha_x$ un ordinal.
        %QV : Où utilises-tu le fait que $Y$ n'a pas de plus grand élément ?
        %HS : J'ai l'impression que c'est utilisé uniquement pour avoir x < \alpha (c.f. ci-dessous)
        Pour tout $x \in Y$ il existe un isomorphisme $f_x$ de $S_{\le x}$ dans $\alpha_x$.
        Si on prend $x < y < \alpha$ alors  $(f_y)_{|S_{\le x}}$ est un isomorphisme et, par unicité, on a donc que $f_y$ prolonge $f_x$.
        %QV : Tu utilises l'axiome du choix, ça peut être intéressant de le faire remarquer.
        %HS : Oui pourquoi pas... à voir s'il y a un intérêt
        On peut définir un isomorphisme $f$ comme union des $f_{x}$ pour $x \in Y$.
        C'est un isomorphisme de $S_{< a}$ dans $\beta := \bigcup_{x < \alpha} \alpha_x$.
        On peut prolonger $f$ en $f_X : S_{\le a} \to \beta + 1$ où $a \mapsto \beta$, d'où $a \le Y$, \textit{\textbf{absurde}}.
    \end{itemize}
  \end{prv}
  ...
  \begin{lem}[Définition récursive transfinie des fonctions]
    ...
    \end{itemize}
    Alors il existe une unique fonction $f$ dans $\mathcal{F}$ définie sur~$\alpha$ telle que :
    \[
      (\star) \quad\quad \text{ pour tout } \beta < \alpha \quad\quad F(f_{|\beta}, f(\beta))
    .\] 
  \end{lem}
  ...
  \begin{prv}
    \begin{description}
      ...
      \item[Existence.]
        ...
        On définit  $f_\tau$ par  $F(f_\gamma, f_\tau(\gamma))$ si  $\gamma < \tau$.
         \begin{itemize}
  ...
  Les axiomes du choix sont exprimable au premier ordre. On donne trois formulations "directes" et deux lemmes.
  Dans la suite de cette section, nous allons montrer que ces énoncés sont équivalents.
  %QV : Est-ce vraiment surprenant ?
  %HS : C'est vrai que c'est peut-être un peu trop enthousiaste
  ...
  \begin{description}
    \item[{\footnotesize(Lemme de)} Zermelo.\label{zermelo}]
      Tout ensemble peut être muni d'un bon ordre (\textit{i.e.} un ordre total où toute partie non vide a un plus petit élément).
  \end{description}
  ...
  On rappelle que l'ensemble $\prod_{i \in I} a_i$ est l'ensemble des fonctions $f$ de $I$ dans $\bigcup_{i \in I} a_i$ telles que $f(i) \in a_i$ pour tout $i$.

  Soit $a$ non vide. On considère $\prod_{\emptyset \neq x \subseteq a}x $ qui est non vide par \ac1.
  Soit~$f$ un de ses éléments.
  ...
  Soit $X = \prod_{i \in I} a_i$ un produit d'ensembles non vides.
  ...
  Le sous-ensemble $\{\beta \in \alpha + 1 \mid F(\beta) = \theta\} $ a un plus petit élément $\beta$.
  Montrons que $F_{|\beta}$ est une bijection de $\beta$ dans $a$.
  \begin{itemize}
  ...
  Soit $Y \subseteq X$ totalement ordonné. Montrons que $Y$ a un majorant dans $X$.
  ...
  \begin{itemize}
    \item S'il existe $x \in a$ tel que $x \cap d = \emptyset$ alors prenons $u \in x$ et posons $d_1 := d \cup \{u\}$.
      On obtient $d_1 \in X$ et $d \subsetneq d_1$ donc $d$ n'est pas maximal, \textit{\textbf{absurde}}.
    \item Pour tout $x \in a$, on a $|d \cap x| = 1$, donc $d$ est l'ensemble recherché (appelé $c$ dans \ac 3).
  \end{itemize}
  ...
  \begin{rmk}[Idée]
    ...
    Ce majorant est l'élément maximal.
  \end{rmk}

  Soit $f : \wp(a) \to a$ une fonction de choix donnée par \ac 2.
  Si $x \subseteq a$ on appelle majorant strict de $x$ un $y \in a$ tel que $z < y$ pour tout~$z \in x$.

  \begin{rmk}[Idée -- suite]
    \begin{itemize}
      \item On part de $\emptyset$ : $a_1 = f(a)$ est un majorant strict de $\emptyset$.
      \item Soit $X_1$ l'ensemble des majorants stricts de $\{a_1\}$. On pose $a_2 := f(X_1)$.
      \item Soit $X_2$ l'ensemble des majorants stricts de $\{a_1,a_2\}$. On pose $a_3 := f(X_2)$.
    \end{itemize}
  \end{rmk}
  ...
  On définit par induction une chaîne maximale (dernier lemme de la section précédente).
  Soit $\theta \not\in a$.
  On définit :
  \[
  F(\alpha) := \begin{cases}
  ...
  L'ensemble $\{F(\beta)  \mid \beta < \alpha_0\}$ n'a pas de majorant strict mais a un majorant $M$ car $a$ inductif.
