...
\begin{document}
  ...
  \begin{defn}["par le bas", "informatique"]
    ...
    \begin{itemize}
      ...
      \item pour tout $n \in \mathbb N$, $\mathcal{F}_{n+1} = \mathcal{F}_n \cup \mleft\{\, 
          \begin{array}{c}
            \lnot F_1\\
  ...
  \begin{thm}[Lecture unique]
    ...
    \label{lem:lecture-unique}
  \end{thm}
  \begin{prv}
    En exercice.

    Une preuve (très, et même probablement trop) détaillée est donnée en annexe, page \pageref{sect:annexe-lem-lecture-unique}.
  \end{prv}
      ...
      \item[Existence.]
        ...
        \begin{itemize}
          ...
          \item si $\bar{\nu}$ est définie sur $\mathcal{F}_n$ alors pour $F \in \mathcal{F}_{n+1}$, on a la disjonction de cas
            \begin{itemize}
              \item si $F = \lnot G$ avec $G \in \mathcal{F}_n$, on définit $\bar{\nu}(F) = 1 - \bar{\nu}(F_1)$ ;
  ...
  \begin{exm}[Table de vérité]
    ...
    \begin{table}[H]
      ...
      \end{tabular}
    \end{table}
    qui liste les différentes assignations possibles de valeurs de vérité aux variables de $F$ et les valeurs de vérité correspondantes pour les sous-formules de~$F$.
  \end{exm}

  \begin{defn}
    \begin{itemize}
      \item Une formule $F$ est dite \textit{satisfaite par une valuation $\nu$} si  $\nu(F) = 1$.
      \item Une \textit{tautologie} est une formule satisfaite par toutes les valuations.
  ...
  \begin{prv}
    ...
    \begin{description}
      \item["$\implies$".]
        Si $\nu$ satisfait toute formule de $\mathcal{E}$, alors tout sous-ensemble fini de $\mathcal{E}$ est satisfait par $\nu$ donc satisfiable.
      \item["$\impliedby$".]
    ...
    \begin{enumerate}
      \item On construit par récurrence une suite $(\varepsilon_n)_{n \ge 1}$ qui satisfait, pour tout $n \in \mathds{N}$,
        \begin{quote}
          "pour toute partie finie $B$ de $\mathcal{E}$, il existe une valuation $\lambda$ satisfaisant  $B$ et telle que pour tout indice~$i \in \llbracket 1, n\rrbracket$, $\lambda(x_i) = \varepsilon_i$".
        \end{quote}
        ...
        \begin{itemize}
          ...
          \item Soit $n \in \mathds{N}$. Supposons que $\varepsilon_1,\ldots,\varepsilon_n$ sont construits et qu'ils satisfont $\mathcal{R}_n$.
            ...
            \begin{itemize}
              ...
              \item Supposons qu'il existe une partie $A$ de $\mathcal{E}$, telle que $(\star)$ toute valuation $\lambda$ satisfaisant $A$ et telle que $\lambda(x_i) = \varepsilon_i$ pour tout  $i \in \llbracket 1,n\rrbracket$ vérifie $\lambda(x_{n+1}) = 1$.
                On pose $\varepsilon_{n+1} := 1$.

                Montrons la propriété $\mathcal{R}_{n+1}$.
                Considérons une partie finie $B$ de $\mathcal{E}$.
                D'après $\mathcal{R}_n$, il existe une valuation $\eta$ qui satisfait $A \cup B$, sous-ensemble fini de $\mathcal{E}$, et qui vérifie $\forall i \in \llbracket 1,n\rrbracket$, $\eta(x_i) = \varepsilon_i$.
                En particulier $\eta$ satisfait $A$, ce qui implique par $(\star)$, que $\eta(x_{n+1}) = 1 = \varepsilon_{n+1}$.
                Et comme $\eta$ satisfait $B$, on obtient bien $\mathcal{R}_{n+1}$.
            \end{itemize}
        \end{itemize}
      ...
      \item Posons $\nu : x_i \mapsto \varepsilon_i$.
        ...
        Soit $F \in \mathcal{E}$.
        On sait que $F$ est un mot (fini), donc qu'il contient un ensemble fini (non vide) de variables.
  ...
  \begin{defn}
    Un ensemble ordonné $(X, \mathcal{R})$ est inductif si tout sous-ensemble $Y$ de $X$ totalement ordonné par $\mathcal{R}$ (\textit{i.e.} une chaîne) admet un majorant dans $X$.
  \end{defn}
  ...
  \begin{exm}
    \begin{enumerate}
      \item $(\mathcal{P}(X), \subseteq )$ est inductif, le majorant d'une chaîne étant l'union de ses éléments.
  ...
  \begin{prv}[Cas non-dénombrable pour le théorème~\ref{chap1-comp}]
    ...
    L'ensemble $\mathcal{V}$ est non-vide car il contient l'application vide de $\{0,1\}^\emptyset$ puisque $\mathcal{E}$ est finiment satisfiable.
    ...
    si $x \in \operatorname{dom} \lambda$ alors il existe $\varphi \in \mathcal{C}$ tel que $x \in \operatorname{dom} \varphi$ et on pose $\lambda(x) = \varphi(x)$.

    La valuation $\lambda$ est définie de manière unique,  \textit{i.e.} ne dépend pas du choix de $\varphi$. En effet, pour tout $x \in \operatorname{dom} \lambda$ et tous $\varphi \in \mathcal{C}$ et $\psi \in \mathcal{C}$ tels que $x \in \operatorname{dom} \varphi \cap \operatorname{dom} \psi$, comme $\mathcal{C}$ est une chaîne on a $\varphi \preceq \psi$ ou $\psi \preceq \varphi$, donc $\varphi(x) = \psi(x)$.
    ...
    L'ensemble de formules $B$ est fini, donc utilise un ensemble fini de variables, dont l'intersection avec $\operatorname{dom}(\lambda)$ est un sous-ensemble fini $\{x_1, \ldots, x_n\}$.
    Il existe $\varphi_1, \ldots, \varphi_n$ dans $\mathcal{C}$ telle que $x_1 \in \operatorname{dom} \varphi_1, \ldots, x_n \in \operatorname{dom} \varphi_n$.
    ...
    On a de plus $x_1, \ldots, x_n \in \operatorname{dom}\varphi_0$.
    On définit :
    \begin{align*}
    ...
    \begin{itemize}
      ...
      \item Si $\gamma'(x) = 1$ alors il existe une partie finie $B_0$ de $\mathcal{E}$ telle que toute valuation $\delta$ prolongeant $\gamma$ et satisfaisant $B_0$ vérifie $\delta(x) = 1$.
  ...
  \section{\textit{Annexe} : preuve détaillée du lemme de lecture unique.}
  \label{sect:annexe-lem-lecture-unique}

  Ce qui suit est une preuve (très, et probablement trop) détaillée du lemme de lecture unique.
  Merci à Quentin Vermande pour cette preuve !

  \begingroup
  \def\thethm{\ref{lem:lecture-unique}}
  \begin{thm}[Lecture unique]
    Toute formule $G \in \mathcal{F}$ vérifie une et une seule de ces propriétés :
    \begin{itemize}
      \item $G \in \mathcal{P}$ ;
  ...
  \begin{prv}
    ...
    Montrons alors que $\mathcal{G}$ est stable par ces mêmes opérations. On a déjà $\mathcal{P} \subseteq \mathcal{G}$.
    ...
    Notons pour toute formule~$F \in \mathcal{F}$, $o(F)$ le nombre de parenthèses ouvrantes de $F$ et $f(F)$ son nombre de parenthèses fermantes et montrons par récurrence simple, pour tout $n \in \mathds{N}$, la propriété 
    \[\mathcal{P}(n) : \quad \forall F \in \mathcal{F}_n, o(F) = f(F).\]
    \begin{itemize}
    ...
    Notons, pour deux mots $u$ et $v$, $u < v$ lorsque $u$ est un préfixe propre de $v$ et montrons par récurrence simple, pour tout $n \in \mathds{N}$, la propriété
    \[\mathcal{P}(n) : \quad \forall F \in \mathcal{F}_n, \forall G < F, o(G) = f(G) = 0 \lor f(G) < o(G).\]
    \begin{itemize}
    ...
    Montrons maintenant par récurrence simple, pour tout $n \in \mathds{N}$, la propriété 
    \[\mathcal{P}(n) : \quad \forall F \in \mathcal{F}_n, \forall G < F, G \notin \mathcal{F}.\]
    \begin{itemize}
    ...
    Par conséquent, $\verb|(| F_1\ \sigma\ F_2 \verb|)| = \verb|(| F_1\ \sigma'\ G_2 \verb|)|$, d'où $\sigma = \sigma'$, puis $F_2 = G_2$.
