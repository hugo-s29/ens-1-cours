...
\begin{document}
  ...
  \begin{rmk}
    ...
    \begin{itemize}
      ...
      \item le principe de récurrence : si on a $\mathcal{P}(0)$ pour une propriété $\mathcal{P}$ et que si $\mathcal{P}(n) \to \mathcal{P}(n+1)$ alors on a $\mathcal{P}(n)$ pour tout $n$.\footnote{C'est le "$\forall \mathcal{P}\: \ldots$" qui pose problème ici.}
    \end{itemize}
  \end{rmk}
  ...
  \begin{exm}
    ...
    \end{itemize}
    On a procédé à un renommage de $y$ à ${\color{nicered} u}$.
  \end{exm}

  \section{Les démonstrations en déduction naturelle.}

  \begin{defn}
    Un \textit{séquent} est un couple noté $\Gamma \vdash F$ (où $\vdash$ se lit "montre" ou "thèse") tel que $\Gamma$ est un ensemble fini de formules appelé \textit{contexte} (\textit{i.e.} l'ensemble des hypothèses), la formule $F$ est la \textit{conséquence} du séquent.
  \end{defn}
  ...
  \begin{rmk}
    ...
    \[
    \begin{prooftree}
      \infer 0[=_\mathsf{i}]{x = 1 \vdash 0 = 0}
      \infer 1[\exists_\mathsf{i}]{x = 1 \vdash \exists x \: x = 0}
      \infer 0[\mathsf{ax}]{x = 1, x = 0 \vdash x = 0}
      \infer 2[\exists_\mathsf{e}]{x = 1 \vdash x = 0}
    \end{prooftree}
    .\] 
  \end{rmk}
  ...
  \begin{nota}
    À la place de $\Val_{\mathcal{M}}(F, e) = 1$, on notera $\mathcal{M}, e \models F$ ou bien~$\mathcal{M} \models F$ (quand $F$ est close).
  ...
  \begin{exm}
    ...
    \end{itemize}
    Aucun des deux n'est un théorème (il n'est vrai que dans les groupes pour $F$ (et c'est même la définition de groupe) et dans les monoïdes pour $G$ (unicité du neutre)), mais $F \to G$ est un théorème logique.
  \end{exm}
  ...
  \begin{defn}
    ...
    \begin{enumerate}
      ...
      \item On dit que $\mathcal{M}$ et $\mathcal{N}$ sont \textit{isomorphes} s'il existe un $\mathcal{L}$-isomorphisme de $\mathcal{M}$ à $\mathcal{N}$.
    \end{enumerate}
  \end{defn}
